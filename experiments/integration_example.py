...
try:
    ...
    print("Warning: Could not import existing config system, using fallback")
    ...
    def launch_and_wrap_env(config):
        # Fallback environment creation
        from experiments.test_unified_avoidance import DemoSimulator
...
def create_avoidance_enabled_env(base_config: Dict[str, Any], avoidance_config: Dict[str, Any] = None):
    ...
    try:
        ...
        logger.warning(f"Failed to use existing env creation: {e}, using fallback")
        from experiments.test_unified_avoidance import DemoSimulator
